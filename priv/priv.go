...
func (privCode *PrivChaincode) Get(stub shim.ChaincodeStubInterface) peer.Response {
	...
	// Read the open data
	dataOpen, err1 := stub.GetPrivateData("DfarmadminDfarmretailOpen", "token")
	if err1 != nil {
		return shim.Error("Error1=" + err1.Error())
	}

	// Read the dfarmadmin private data
	dataSecret, err2 := stub.GetPrivateData("DfarmadminPrivate", "token")
	...
	// Returns the token value from 2 PDC + error in retrieving from DfarmadminPrivate collection
